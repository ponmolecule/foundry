...
def main():
    ...
    ck("paste-surface inventory is explicit and complete", len(paste_lines) == 28, str(len(paste_lines)))
    ck("every paste textarea has live per-box activation wiring", all("oninput=" in ln for ln in paste_lines))

    # Twenty-four scalar schedule families: eleven Fee Product paths (transaction coefficient, Transaction fee rate, Flat amount,
    ...
    # driver/attrition, Workforce Count/Compensation, individual Operating Expense schedule, and
    # headcount-linked Opex amount-per-FTE, plus Formula / level Fixed Asset base / multiplier / depreciation paths, and Formula / driver Opex factors.
    scalar_markers = [
        ...
        "wfCountPaste_", "wfCompPaste_", "nieCatExplicitPaste_", "nieLinkedWfAmount_", "feeCostFactorPaste_", "feeCostMultiplierPaste_",
        "secsp_", "fasp_", "opexSvc_", "opexSvcCap_", "opexFormulaFactor_",
    ]
    ck("all twenty-four scalar Explicit schedule families are present", all(x in html for x in scalar_markers))
...
"""
    ...
    scalar_load_fragments = [
        ...
        'id="${id}_load" class="pillbtn" disabled onclick="secSeriesPaste',
        'id="${id}_load" class="pillbtn" disabled onclick="faSeriesPaste',
        'id="${id}_load" class="pillbtn" disabled onclick="nieCatServiceLevelPaste',
        'id="${id}_load" class="pillbtn" disabled onclick="nieCatServiceCapacityPaste',
        'id="${id}_load" class="pillbtn" disabled onclick="nieCatFormulaExplicitPaste',
    ]
    ...
    clear_markers = [
        ...
        "nieCatScheduleClear", "nieCatLinkedAmountClear", "secSeriesClear", "faSeriesClear",
        "nieCatServiceLevelClear", "nieCatServiceCapacityClear", "nieCatFormulaExplicitClear",
    ]
