...
DEP_FIELDS = [
    ...
    ("index", "Reference index (float only)", "sofr | effr | prime  (blank = sofr)"),
    ...
    ("__RATE__", "Rate paid / Spread over index (% p.a.)",
     "fixed: annual rate (4.90% = 0.049).  float: spread over the chosen index, may be negative"),
...
LEND_FIELDS = [
    ...
    ("rate_type", "Rate type", "fixed | float  (editable)"),
    ("index", "Reference index (float only)", "sofr | effr | prime  (blank = sofr)"),
    ("__RATE__", "Yield / Spread over index (% p.a.)",
     "fixed: annual yield (6.50% = 0.065).  float: spread over the chosen index, may be negative"),
    ("__RATE_BASELINE__", "Interpreted as (live)", "fact — informational, follows the Rate type dropdown, not imported"),
