    ...
    if _cac_cfgs:
        ...
        for _nm, _fcfg in _cac_cfgs.items():
            ...
            _cac_out[_nm] = {
                ...
                "yearEndCustomers": [float(v or 0.0) for v in (_cr.get("year_end_customers") or [])],
                "customerEndByPeriod": [float(v or 0.0) for v in (_cr.get("customer_end_by_period") or [])],
                "customerEndByMonth": [float(v or 0.0) for v in (_cr.get("customer_end_by_month") or [])],
                "customerLevelByPeriod": [float(v or 0.0) for v in (_cr.get("customer_level_by_period") or [])],
                "customerAverageByMonth": [float(v or 0.0) for v in (_cr.get("customer_average_by_month") or [])],
                "customerAverageByPeriod": [float(v or 0.0) for v in (_cr.get("customer_average_by_period") or [])],
                "customerAnnualCountByMonth": [float(v or 0.0) for v in (_cr.get("customer_annual_count_by_month") or [])],
