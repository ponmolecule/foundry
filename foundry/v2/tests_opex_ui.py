...
checks=[
 ...
 ('tiered Opex UX separates timing, composite driver, and band schedule into delineated sections', 'opex-piecewise-section' in html and 'Timing & recognition' in html and 'Build the balance used by the band schedule.' in html and 'Amounts are in $000s.' in html),
 ('tiered Opex timing controls share a purpose-built aligned grid', 'class="opex-piecewise-timing-grid"' in html and 'First cash event' in html and re.search(r'\.opex-piecewise-timing-grid\{[^}]*grid-template-columns:minmax\(190px,1\.2fr\) minmax\(140px,\.8fr\) minmax\(165px,\.9fr\)[^}]*align-items:end',html) is not None),
 ('tiered Opex band inputs use compact one-row columns with same-row remove action', 'class="opex-piecewise-band-row"' in html and 'class="btn-plain opex-piecewise-remove" onclick="nieCatPiecewiseRemoveBand' in html and re.search(r'\.opex-piecewise-band-head,\.opex-piecewise-band-row\{[^}]*grid-template-columns:minmax\(0,112px\) minmax\(0,112px\) minmax\(0,112px\) minmax\(0,170px\) auto',html) is not None),
 ...
 ('tiered Opex components have card-level visual separation for dense multi-component authoring', 'class="opex-cost-pool-editor opex-piecewise-card"' in html and re.search(r'\.opex-piecewise-card\{[^}]*margin:18px 0[^}]*padding:12px 13px 14px[^}]*border:1px solid rgba\(148,163,184,\.32\)[^}]*background:rgba\(20,31,48,\.28\)',html) is not None and '.opex-piecewise-card + .opex-piecewise-card{margin-top:24px}' in html),
 ('tiered Opex marginal-rate column is wider for long decimal coefficients', 'Marginal rate (decimal)' in html and 'minmax(0,170px)' in html),
 ...
 ('service-capacity Opex keeps external FTE separate from Workforce and exposes explicit capacity period', '+ Service-capacity component' in html and 'Service-capacity component' in html and 'Expense = service FTE × hourly rate × periodized hours/FTE.' in html and 'does not change Workforce Count or payroll' in html and 'Hours / FTE' in html and 'nieCatServiceCapacityPeriod' in html),
 ...
 ('linked Opex percent field stays compact and the delete action cannot wrap onto a second line', 'class="crow opex-linked-component-row"' in html and 'class="opex-linked-rate"' in html and 'class="cx opex-linked-delete"' in html and re.search(r'\.opex-linked-component-row\{[^}]*flex-wrap:nowrap[^}]*overflow-x:auto',html) is not None and re.search(r'\.opex-linked-component-row \.opex-linked-rate\{[^}]*width:62px !important[^}]*flex:0 0 62px',html) is not None and re.search(r'\.opex-linked-component-row \.opex-linked-delete\{[^}]*flex:0 0 auto',html) is not None),
