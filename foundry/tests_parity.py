...
def main():
    ...
    try:
        ...
        need_v31 = ["window.V31", '"start","Start"', "renderStart", "openEngagement",
                    ...
                    "Two ways in, one model", "TAXONOMY_V31", "loanCRE", "addFromTaxonomy", "materializeTemplate", "syncModules", "wizStart", "renderWizard", "Day-1 arithmetic", "REGULATOR_OF"]
        miss += [x for x in need_v31 if x not in html]
        for tok in ['"/v3.1"', '"/api/v31/engagement/{slug}"', '"/api/v31/fieldlib"', '"/api/v31/template"', '"/api/v31/fiw"', '"/api/v31/fiw/import"', '"/api/v31/modet/recon"', '"/v3"', '"/v2.2"', 'docs_url=None', '"/api/v2/freeze"', '"/api/v2/verify/{entry_id}"']:
