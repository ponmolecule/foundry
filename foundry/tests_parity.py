...
def main():
    ...
    try:
        ...
        need_v31 = ["window.V31", '"start","Start"', "renderStart", "openEngagement",
                    ...
                    "Two ways in, one model", "TAXONOMY_V31", "loanCRE", "addFromTaxonomy", "materializeTemplate", "syncModules", "wizStart", "renderWizard", "mtUpload", "mtRender", "mtFinalize", "Entry screen", "Defaults in effect (not from the client)", "FOMC SEP", "Retrodiction", "retroUpload", "/api/v31/retro", "CharterIQ substrate", "retroSubstrate", "CHARTERIQ_DATABASE_URL", "Staged raises", "capital_raises", "subPlace", "Place modeled capital vs real peers", "vintageBuild", "Vintage corridor", "/api/v31/substrate/vintage", "Pre-opening", "pre_opening", "Min Day-1 capital", "Securities books", "aoci_sensitivity_annual", "secAdd", "class=\"cbl\"", "class=\"crow\"", "Day-1 arithmetic", "REGULATOR_OF"]
        miss += [x for x in need_v31 if x not in html]
        for tok in ['"/v3.1"', '"/api/v31/engagement/{slug}"', '"/api/v31/fieldlib"', '"/api/v31/template"', '"/api/v31/fiw"', '"/api/v31/fiw/import"', '"/api/v31/modet/recon"', '"/api/v31/modet/finalize"', '"/v3"', '"/v2.2"', 'docs_url=None', '"/api/v2/freeze"', '"/api/v2/verify/{entry_id}"']:
