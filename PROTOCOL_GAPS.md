...
## r68 addendum — compositional Opex and CAC-owned customer counts
The r68 change removes the r67 authoring special case without breaking saved r67 economics. New Operating Expense authoring treats a cost-pool/cost-recovery charge as a typed additive component alongside the ordinary entered recurring expense and other linked components. Legacy r67 `calculation.kind = cost_pool` configurations remain readable with their original exclusive behavior so dormant drafts are not reactivated. Separately, Customer Acquisition publishes a stable customer-count Series and Account Fee streams may consume it directly; CAC remains the sole owner of the count path and intra-year shape, while the Fee Product owns only its per-client price path. No Platform Integration, Reg W, intercompany, or engagement-specific label is part of either runtime ontology. Profile B's broader first-class Fee Product limitation remains unchanged.
