...
**Managed securities portfolios.** Configuration → Securities books & AOCI also supports a target-driven stock/flow grammar alongside the legacy simple books. A portfolio links to a stable balance-sheet target Series (currently Total Equity · period end or Total Deposits · period end) and applies an authored target ratio. Named sleeves then carry authored AFS/HTM classification, allocation %, opening balance, maturity/runoff assumptions, and either an entered annual-yield path or an optional Curve Library source. Instrument names, target ratios, allocations, runoff rates, and yields are data — none are hard-coded. For each engine period: `target sleeve = linked target × target ratio × allocation`; `maturing = starting × effective runoff`; `net purchases/(sales) = target sleeve − starting + maturing`; `ending = starting − maturing + net purchases/(sales)`. Net purchases remain signed (negative means sale/reduction). Runoff owns an explicit Month/Quarter/Year natural period and is converted by survival-equivalence; its trajectory cadence is a separate concept. Interest income is `ending × annual yield / periods_per_year`, so monthly books apply the economically required `/12`. When current-period Equity drives the target, the existing fixed-point solve resolves the Equity → securities → interest/AOCI → net-income → Equity dependency rather than substituting prior-period equity. Managed AFS balances participate in AOCI; HTM balances remain AOCI-immune under the existing classification semantics.
