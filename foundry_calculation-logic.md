...
**Managed securities portfolios.** Configuration → Securities books & AOCI also supports a target-driven stock/flow grammar alongside the legacy simple books. A portfolio links to a stable balance-sheet target Series (currently Total Equity · period end or Total Deposits · period end), owns an explicit target timing (Current period or Prior period), and applies an authored target ratio. Prior-period timing also owns an explicit first-model-period initialization (`zero` or `opening_source`) so the engine never silently substitutes an opening balance. Named sleeves then carry authored AFS/HTM classification, allocation %, opening balance, maturity/runoff assumptions, and either an entered annual-yield path or an optional Curve Library source. Instrument names, target ratios, allocations, runoff rates, and yields are data — none are hard-coded. For each engine period: `target sleeve = timed linked target × target ratio × allocation`; `maturing = starting × effective runoff`; `net purchases/(sales) = target sleeve − starting + maturing`; `ending = starting − maturing + net purchases/(sales)`. Net purchases remain signed (negative means sale/reduction). Runoff owns an explicit Month/Quarter/Year natural period and is converted by survival-equivalence; its trajectory cadence is a separate concept. Interest income is `ending × annual yield / periods_per_year`, so monthly books apply the economically required `/12`. Current-period Equity targets remain inside the fixed-point solve; prior-period targets consume the already-finalized previous balance-sheet period and therefore break that same-period circularity. When managed portfolios are present, their explicit books own the securities allocation: the legacy funding waterfall no longer creates an additional residual securities plug on top of them, and surplus funding remains in cash. Managed AFS balances participate in AOCI; HTM balances remain AOCI-immune under the existing classification semantics.
...
**Overhead** uses the canonical periodic-flow contract for simple/detailed Operating Expense. If granular NIE is present, overhead is built from Workforce compensation, entered Opex category flows, optional typed additive components (including read-only transaction Fee Stream throughput, Workforce Count × amount-per-FTE links, shared cost-pool / cost-recovery charges, and generic tiered / banded linked components), regulatory assessments, and depreciation, plus the configured "other" gross-up `sub × r/(1−r)`. Workforce Count-linked Opex consumes either a role/population Count Series or the Workforce-owned aggregate `total_count_series_id`; the coefficient is a Flat/Growth/Explicit dollar amount per FTE with a Month/Quarter/Year natural period. It is periodized once, then multiplied by the resolved active headcount for the same engine period. The aggregate count is derived from role Count Series used by payroll rather than authored independently, and the link is keyed by stable Series ID rather than role name. A tiered / banded component composes one or more upstream balance terms, observes them with an explicit ordinal lag, and applies a piecewise-linear table of `literal band base fee + marginal rate × (driver − band lower bound)` on a monthly/quarterly/semiannual/annual event cadence. Band base fees are entered literals and are never smoothed or derived from the preceding band, so source schedules that round or rebase at breakpoints are preserved exactly. Tiered/banded recognition is component-owned: legacy/missing recognition means **At cash event**; `spread` recognition calculates one event amount for the cadence interval, recognizes it evenly over an explicitly anchored coverage interval, settles the full amount at the configured cash event, and routes the timing difference to Prepaid operating expenses / Accrued operating expenses. Spread recognition fails closed unless the configured observation is already available at the beginning of the covered interval, preventing future-balance look-ahead. Supported balance terms are generic module-owned observations: bank Total Assets, Customer Acquisition AUC with explicit Period end / Period average measure, and stable driver-quantity Series published by Balance Fee streams. A typed cost-pool Opex component contributes the downstream charge exactly once as `eligible cost pool × recovery % × (1 + markup %)` and composes with the category's ordinary entered expense and other additive components; the pool's linked, entered, and balance-derived components are calculation inputs and do not themselves create an additional Opex posting. Legacy r67 `calculation.kind = cost_pool` configurations retain their original exclusive semantics only for backward compatibility. The same direction-agnostic cost-pool primitive may instead be consumed by a Fee Product when the modeled bank is the service provider and the intercompany charge is revenue. Customer Acquisition calculates its acquisition equations on a canonical monthly causal grid and publishes stable customer-count and AUC Series from those monthly results. Month / Quarter / Year are source-assumption cadences, not a module-wide annual calculation cadence: natural-period flow operands are distributed into their constituent months, level/rate operands hold until their authored path changes, and linked upstream Series are consumed on their owner-resolved monthly path without double periodization. Annual CAC tables are aggregations of the monthly engine, not the source of the monthly path. Legacy CAC within-year shape fields remain readable but no longer manufacture customer/AUC timing after an annual equation. Account Fee streams may consume the CAC-owned count directly and apply their own per-account price path without duplicating the client forecast. The downstream stream explicitly chooses `annual_count`, canonical-month `period_end`, or `period_average` active-client exposure. Quarterly engines aggregate the underlying monthly count measure before applying an annual fee so changing projection cadence does not silently rewrite the economics. Saved r68 streams with no count measure retain the prior canonical-month EOP interpretation. Balance-derived cost-pool components are hard-typed to Customer Acquisition's canonical AUC Series (`feed.series_id`, semantic `auc_end`); a CAC `customer_count_series_id` is never a valid monetary balance source. The authoring UI previews the selected pre-multiplier AUC measure separately from the downstream `resolved eligible cost`, preventing a calculated dollar-flow preview from being mistaken for the underlying balance Series. **FDIC** continues to accrue from its configured annual bp assumption on `max(0, assets − tangible equity)`. The old simplified **OCC** bp/year shortcut is now legacy/opt-in only: modern Detailed Opex with no explicit simplified-OCC enablement posts zero OCC shortcut expense. Pre-r78 saved models that persisted `occ_bp_ann` remain backward-compatible, and an explicitly enabled shortcut still applies one-half of the annualized rate to one measurement base per half-year and settles on its ordinal cash-payment cadence. Source-faithful assessments should instead use the generic Advanced tiered / banded component with spread recognition when the assessment covers a service period rather than belonging wholly to the payment month. Category recognition and cash-settlement timing belong only to the entered recurring Opex path; additive components retain their own/native timing except for self-timed mechanics such as tiered/banded recognition. For the entered recurring path, settlement remains separate from recognition and prepayment/accrual differences surface as Prepaid operating expenses / Accrued operating expenses on the Balance Sheet. Fee-product operating costs remain separately presented. **Net fee income** is an internal management driver, not a Call Report presentation rewrite: `Net fee income = Total noninterest income − Fee Product Costs`. The Formula / driver and ordinary linked-Opex pickers may consume this same-period derived base (for example, an IP licensing fee equal to 10% of Net fee income) while the Income Statement continues to report Total noninterest income gross and Fee Product Costs separately. Because the deduction is the canonical Fee Product Costs line, any direct product cost routed there—including Cost of Risk when modeled there—reduces Net fee income before downstream percentage-based Opex is calculated. Transaction Fee streams separate **quantity construction** from **revenue pricing**. Revenue pricing has two explicit bases: `per_unit`, where a transaction-count quantity is multiplied by a raw dollar amount per transaction, and `pct_of_throughput`, where monetary throughput is multiplied by a decimal rate. The pricing basis is independent of how the transaction quantity was derived; a turns coefficient does not imply percentage pricing. Saved pre-r133 derived Transaction streams that do not carry `pricing_basis` preserve their historical interpretation (`pct_of_throughput` when a derived coefficient exists, otherwise `per_unit`) rather than being silently migrated. Transaction Fee streams support three typed derived coefficients: `multiple` (turns × source), `pct` (% of source), and `amount_per_source_unit` (monetary throughput per source unit). `multiple` and `amount_per_source_unit` are natural-period flow coefficients and are periodized once from Month / Quarter / Year into the native engine period before the transaction fee/spread is applied. A `pct` coefficient declares one of two semantics: `share` is a dimensionless attach/migration/penetration factor whose percentage is never divided by projection cadence, while `flow` is a natural-period flow ratio (for example annual transaction volume as % of AUC) and is periodized like turns. For a `share`, Month / Quarter / Year controls only the authored trajectory or explicit-schedule timing, not the magnitude of the percentage. Pre-r101 `pct` coefficients without a semantics marker are resolved structurally and without engagement-name rules: count-sourced chains infer `share`; monetary or otherwise native sources retain the historical `flow` behavior. The amount-per-source-unit path is authored in `$000s per source unit`, stored internally in dollars, and periodized once into the native engine period. `multiple` and `pct` preserve the upstream source unit; amount-per-source-unit converts the resulting throughput into monetary dollars. Fee Product costs preserve a two-layer contract: the original scalar cost (`cost_per_unit`, `% of gross fee revenue`, or for Transaction streams `% of transaction throughput`) remains the base economic assumption, and an optional dimensionless cost multiplier may be authored as Flat, Growth, or Explicit. Throughput-based operating cost is calculated as resolved Transaction throughput × cost rate and routes to Fee Product Costs/NIE without reducing gross fee income; it is not reverse-engineered as an equivalent percentage of revenue. The selected Month/Quarter/Year period governs only the multiplier trajectory: the resolved multiplier is applied to each engine-period base cost as a level and is never divided by periods-per-year. For an Explicit Year/Step path, the Year-1 multiplier applies unchanged to every model period in Year 1, the Year-2 multiplier to every model period in Year 2, and so on. With no multiplier path, the implicit multiplier is exactly `1.0`, preserving legacy scalar economics. Saved r82 `factor_path` configurations remain readable under their original replacement semantics, but new authoring uses the separate multiplier layer.
...
**r73 authoring-state clarification.** Cost pools remain direction-neutral global calculation objects for engine purposes, but new Operating Expense authoring treats a newly-created pool as private to the specific Opex cost-pool component until the user explicitly links another consumer. The Opex editor does not enumerate the global pool registry by default; existing pools appear only behind an explicit sharing action, and only in-use pools are offered. A private Opex-owned pool is removed when its sole consumer is removed or repointed, preventing stale assumptions from resurfacing in later expense authoring. This ownership metadata affects authoring lifecycle only and does not change cost-pool arithmetic or posting semantics. Generic model-authoring paste surfaces use presentation-only Close/Edit state; generated controls are excluded from native-close detection so Close cannot cancel itself on the next decorator pass.


**r74 Opex authoring-state clarification.** Cost-pool economics remain global and direction-neutral, but Operating Expense authoring is private-by-default. A newly-created typed Opex cost-pool component shows only its attached pool; reuse of a live shared pool is an explicit action. UI disclosure state is keyed by stable category/component identity rather than row position. Opex-owned pools with no remaining consumer are cleaned when their owning category/component is removed, and Detailed-mode migration hygiene prunes orphaned Opex-owned records leaked by prior authoring releases. This lifecycle cleanup does not alter any live/shared pool, cost-pool arithmetic, balance measure, recovery/markup formula, or NIE posting. The Opex editor also hard-contains long Series labels and AUC previews; the generic CAC preview offset is disabled within Opex only.

**r76 Opex AUC-preview clarification — no calculation change.** An ordinary AUC-linked Operating Expense component now previews the exact canonical monthly balance measure selected by the consumer. `Period end` displays monthly EOP AUC; `Period average` displays `(prior month-end + current month-end) / 2`, using the configured opening AUC for the first month. This changes authoring/audit presentation only: the Opex engine already accrued from the selected monthly measure and then aggregated the resulting monthly expense to presentation cadence.

**r77 generalized tiered-Opex clarification.** Detailed Operating Expense Advanced authoring supports an additive `Tiered / banded component` rather than a regulator- or engagement-specific assessment object. The component sums weighted upstream balance observations, applies an explicit observation lag, and evaluates a literal-base piecewise-linear schedule only on its configured ordinal event cadence. Total Assets requires a positive lag so the expense cannot depend circularly on the same period's balance sheet; natural lags finer than the selected model cadence fail closed. Balance Fee streams publish stable observational quantity Series so another module may consume an already-modeled stock without re-authoring its assumptions; Profile B fails closed for that term because it does not yet expose first-class Fee Product quantity history. The pre-existing simplified OCC bp/year assessment remains available only as a legacy/quick-estimate shortcut. From r78 onward it is OFF by default in newly authored Detailed Opex; absence means zero shortcut expense. Pre-r78 saved models that actually persisted an OCC rate continue to calculate it for backward compatibility. When explicitly enabled, the shortcut remains additive to Advanced tiered / banded components.


## r78 simplified-OCC default-off clarification
Detailed Operating Expense no longer creates an OCC charge merely because the module is active. New Detailed-Opex objects carry `occ_simplified_enabled=false` and do not materialize a default OCC rate. The engines no longer fall back from a missing `occ_bp_ann` to the regulatory-parameter reference value. The simplified semiannual bp/year mechanic remains available behind an explicit `Legacy / simplified OCC assessment` enable control, and its first-payment field is labeled as cash timing rather than recognition timing.

Backward compatibility is presence-based: a pre-r78 saved model with `occ_bp_ann` but no enable flag is inferred enabled and retains its prior semiannual recognition/payment economics. An explicit `occ_simplified_enabled=false` always wins, even if a legacy rate remains stored as draft state. The generic Advanced tiered / banded component remains the preferred source-faithful assessment mechanism.
